...
class RubyLLM::Modes::RouterCallTest < Minitest::Test
  ...
  def test_explicit_route
    ...
    assert_equal "explicit", route.level
    assert_equal "Explicit mode requested", route.reason
  ...
  def test_fallback_only_shortcut_skips_the_classifier
    ...
    assert_equal TutorAgent, route.mode
  ...
  def test_routes_only_to_available_modes
    [ true, false ].each do |enabled|
      ...
      assert_includes ThresholdRouter.new(showtime_enabled: enabled).modes.map(&:klass), route.mode
    end
  end
